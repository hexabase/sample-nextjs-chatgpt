...
export const getOpenAiRequestOptions = ({ apiKey, model, ...restOfApiParams }: OpenAIStreamingParams, messages: OpenAIChatMessage[], signal?: AbortSignal): FetchRequestOptions => ({
  headers: {
...
const CHAT_COMPLETIONS_URL = `${process.env.NEXT_PUBLIC_CHAT_GPT_API_URL}/chat/completions`;
...
export const openAiStreamingDataHandler = async (
  ...
  for await (const newData of streamAsyncIterator(response.body) as unknown as NodeJS.ReadableStream) {
    // Decode the data
